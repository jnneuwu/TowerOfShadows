...
public static class PrototypeBuilder
{
    ...
    static void CreateLevelScene(PrototypeData.LevelDef level, PrototypeAssets.Bundle bundle)
    {
        ...
        // Lab 6: shared FX assets that runtime particle/pickup code reads from
        GameObject fxObj = new GameObject("FxAssets");
        var fxHolder = fxObj.AddComponent<FxAssetHolder>();
        fxHolder.lightSprite = bundle.lightSprite;
        fxHolder.weaponIconSprite = bundle.weaponIconSprite;
        ...
        // Lab 6: attach a soft light to the player so they're always visible
        AttachPlayerLight(player.transform, bundle.lightSprite);
        ...
        List<RoomDoor> allDoors = CreateDoors(level, bundle.wallTopTile.sprite);
        for (int d = 0; d < level.Doors.Length; d++)
        {
            string key = level.Doors[d].RoomKey;
            if (roomManagers.ContainsKey(key))
                roomManagers[key].doorsToOpen.Add(allDoors[d]);
        }
        ...
        {
            foreach (PrototypeData.ChestSpawn chest in level.Chests)
            {
                GameObject chestObj = (GameObject)PrefabUtility.InstantiatePrefab(bundle.weaponChestPrefab);
                chestObj.transform.position = chest.Position;
                WeaponChest wc = chestObj.GetComponent<WeaponChest>();
                if (wc != null) wc.weaponType = (WeaponData.WeaponType)chest.WeaponIndex;
                // Pulsing pickup glow so chests are visible
                AttachPickupGlow(chestObj.transform, bundle.lightSprite, new Color(1f, 0.8f, 0.3f), 1.6f);
            }
        }

        // Lab 6: scatter wall torches around the level so we have multiple light sources
        SpawnTorchesForLevel(level, bundle);

        EditorSceneManager.SaveScene(SceneManager.GetActiveScene(), SceneFolder + "/" + level.SceneName + ".unity");
    }

    // ----- Lab 6: lighting helpers -----
    static void AttachPlayerLight(Transform player, Sprite lightSprite)
    {
        if (lightSprite == null) return;
        GameObject l = new GameObject("PlayerLight");
        l.transform.SetParent(player, false);
        var sr = l.AddComponent<SpriteRenderer>();
        sr.sprite = lightSprite;
        sr.sortingOrder = 50;
        var lit = l.AddComponent<SimpleLight>();
        lit.lightColor = new Color(1f, 0.95f, 0.75f);
        lit.radius = 5f;
    }

    static void AttachPickupGlow(Transform parent, Sprite lightSprite, Color color, float radius)
    {
        if (lightSprite == null) return;
        GameObject l = new GameObject("Glow");
        l.transform.SetParent(parent, false);
        var sr = l.AddComponent<SpriteRenderer>();
        sr.sprite = lightSprite;
        sr.sortingOrder = 7;
        var lit = l.AddComponent<SimpleLight>();
        lit.lightColor = color;
        lit.radius = radius;
        l.AddComponent<PulsingLight>();
    }

    static void SpawnTorchesForLevel(PrototypeData.LevelDef level, PrototypeAssets.Bundle bundle)
    {
        if (bundle.torchSprite == null || bundle.lightSprite == null) return;

        // Keep it dark and atmospheric: one torch per side of the top wall.
        // Two torches per room is plenty - any more and the dungeon stops feeling dark.
        foreach (var room in level.Rooms)
        {
            if (room.Key == "Spawn") continue;
            float minX = room.Area.xMin;
            float w = room.Area.width;
            float topY = room.Area.yMax - 0.8f;

            SpawnTorch(new Vector2(minX + w * 0.25f, topY), bundle);
            SpawnTorch(new Vector2(minX + w * 0.75f, topY), bundle);
        }
    }

    static void SpawnTorch(Vector2 pos, PrototypeAssets.Bundle bundle)
    {
        GameObject torch = new GameObject("Torch");
        torch.transform.position = pos;
        var bodySR = torch.AddComponent<SpriteRenderer>();
        bodySR.sprite = bundle.torchSprite;
        bodySR.sortingOrder = 6;

        GameObject l = new GameObject("Light");
        l.transform.SetParent(torch.transform, false);
        var lsr = l.AddComponent<SpriteRenderer>();
        lsr.sprite = bundle.lightSprite;
        lsr.sortingOrder = 50;
        var sl = l.AddComponent<SimpleLight>();
        sl.lightColor = new Color(1f, 0.65f, 0.25f);
        sl.radius = 4f;
        l.AddComponent<TorchLight>();   // dynamic flicker
    }
    ...
    static List<RoomDoor> CreateDoors(PrototypeData.LevelDef level, Sprite sprite)
    {
        List<RoomDoor> list = new List<RoomDoor>();
        ...
        foreach (PrototypeData.DoorDef door in level.Doors)
        {
            ...
            list.Add(doorObject.AddComponent<RoomDoor>());
        }
        return list;
    }
    ...
    static GameObject CreateEnemy(PrototypeData.EnemyKind kind, PrototypeAssets.Bundle bundle)
    {
        ...
        if (kind == PrototypeData.EnemyKind.Floor1Boss)
        {
            ...
            enemy.transform.localScale = new Vector3(1.6f, 1.6f, 1f);   // bigger and scarier
            ShadowCharger boss = enemy.GetComponent<ShadowCharger>();
            boss.maxHP = 400;          // up from 140
            boss.currentHP = 400;
            boss.contactDamage = 25;
            boss.chargeDamage = 60;    // up from 35
            boss.chargeCooldown = 1.4f; // faster charges (was 2.2)
            boss.chargeSpeed = 16f;     // faster than default 12
        ...
        else if (kind == PrototypeData.EnemyKind.Floor2Boss)
        {
            ...
            enemy.transform.localScale = new Vector3(1.6f, 1.6f, 1f);
            ShadowCaster boss = enemy.GetComponent<ShadowCaster>();
            boss.maxHP = 550;           // up from 180
            boss.currentHP = 550;
            boss.contactDamage = 20;
            boss.fireInterval = 0.85f;  // way faster (was 1.6)
            boss.orbDamage = 40;         // up from 25
            boss.orbSpeed = 7f;          // faster (was 5.5)
        ...
        else if (kind == PrototypeData.EnemyKind.FinalBoss)
        {
            ...
            boss.maxHP = 1000;          // explicit at scene level too (prefab is 800)
            boss.currentHP = 1000;
            boss.isFloorBoss = true;
        }
    ...
    static void BuildTilemap(PrototypeData.LevelDef level, PrototypeAssets.Bundle bundle)
    {
        ...
        HashSet<Vector3Int> firstRing = new HashSet<Vector3Int>();
        foreach (Vector3Int floor in floorCells)
        {
            ...
            for (int dy = -1; dy <= 1; dy++)
            {
                ...
                if (!floorCells.Contains(candidate))
                {
                    ...
                    firstRing.Add(candidate);
                }
            }
        }
        foreach (Vector3Int wall in firstRing)
        {
            for (int dx = -1; dx <= 1; dx++)
            for (int dy = -1; dy <= 1; dy++)
            {
                Vector3Int candidate = new Vector3Int(wall.x + dx, wall.y + dy, 0);
    ...
    static void BuildGameplayUi(Transform canvas)
    {
        ...
        ui.ammoText = CreateText(uiObject.transform, "AMMO: 30 / 30", new Vector2(48f, 42f), new Vector2(360f, 42f), TextAnchor.MiddleLeft, 32, new Vector2(0f, 0f), new Color(0.96f, 0.96f, 1f));
        ui.weaponNameText = CreateText(uiObject.transform, "Pistol", new Vector2(48f, 78f), new Vector2(360f, 32f), TextAnchor.MiddleLeft, 22, new Vector2(0f, 0f), new Color(1f, 0.85f, 0.3f));
        ui.interactPromptText = CreateText(uiObject.transform, "Press E to open", new Vector2(0f, -120f), new Vector2(500f, 40f), TextAnchor.MiddleCenter, 26, new Vector2(0.5f, 0.5f), new Color(1f, 0.95f, 0.6f));
        ...
        // Top-left debug panel: State / Speed / Direction / Aim / Grounded.
        Color debugColor = new Color(0.4f, 1f, 0.7f);
        ui.stateText     = CreateText(uiObject.transform, "State: Idle",      new Vector2(20f, -24f),  new Vector2(360f, 26f), TextAnchor.UpperLeft, 20, new Vector2(0f, 1f), debugColor);
        ui.speedText     = CreateText(uiObject.transform, "Speed: 0.0",       new Vector2(20f, -50f),  new Vector2(360f, 26f), TextAnchor.UpperLeft, 20, new Vector2(0f, 1f), debugColor);
        ui.directionText = CreateText(uiObject.transform, "Direction: Right", new Vector2(20f, -76f),  new Vector2(360f, 26f), TextAnchor.UpperLeft, 20, new Vector2(0f, 1f), debugColor);
        ui.aimText       = CreateText(uiObject.transform, "Aim: 0 deg",       new Vector2(20f, -102f), new Vector2(360f, 26f), TextAnchor.UpperLeft, 20, new Vector2(0f, 1f), debugColor);
        ui.groundedText  = CreateText(uiObject.transform, "Grounded: True",   new Vector2(20f, -128f), new Vector2(360f, 26f), TextAnchor.UpperLeft, 20, new Vector2(0f, 1f), debugColor);

        // Bottom-right music / SFX toggle indicators (Lab 5).
        ui.musicIndicatorText = CreateText(uiObject.transform, "[M] Music: ON", new Vector2(-20f, 60f), new Vector2(280f, 28f), TextAnchor.MiddleRight, 20, new Vector2(1f, 0f), new Color(0.4f, 1f, 0.5f));
        ui.sfxIndicatorText   = CreateText(uiObject.transform, "[N] SFX: ON",   new Vector2(-20f, 30f), new Vector2(280f, 28f), TextAnchor.MiddleRight, 20, new Vector2(1f, 0f), new Color(0.4f, 1f, 0.5f));
