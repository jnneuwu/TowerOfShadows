...
public class ShadowCharger : EnemyBase
{
    ...
    {
        base.Start();
        // Only apply class defaults when the value is still the EnemyBase fallback.
        // This lets PrototypeBuilder customise boss stats without being overwritten here.
        if (maxHP == 20) { maxHP = 80; currentHP = maxHP; }
        if (contactDamage == 10) contactDamage = 15;
        if (detectionRange == 6f) detectionRange = 8f;
    }
